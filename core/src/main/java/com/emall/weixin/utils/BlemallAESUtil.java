...
//	private final static String HEX = "1234567890123456";
	private final static Logger logger = LoggerFactory.getLogger(BlemallAESUtil.class);
	...
	public static String decrypt(String key,String decryptStr){
		try {
			...
			//偏转向量
			IvParameterSpec iv = new IvParameterSpec(HexStringToBinary(ivHexStr));
			...
			return new String(cipher.doFinal(HexStringToBinary(decryStr)), "UTF-8").trim();
		} catch (NoSuchAlgorithmException e) {
			logger.error("------>",e);
		} catch (NoSuchPaddingException e) {
			logger.error("------>",e);
